...
<#
.SYNOPSIS
  Tests for Write-DetectInstalledOutput in delphi-toolchain-inspect.ps1

.DESCRIPTION
  Covers: text and JSON output formatting for -DetectInstalled results.
  ...
  Context 6 - JSON, DCC mode:
    ok=true, command=detectInstalled, result.platform/buildSystem/installations.
...
Describe 'Write-DetectInstalledOutput' {
  ...
  Context 'Text, DCC, all entries notFound or notApplicable' {
    ...
    BeforeAll {
      $script:out = Write-DetectInstalledOutput `
  ...
  Context 'JSON, DCC mode - structure and readiness distinctions' {
    ...
    BeforeAll {
      $rawOut = Write-DetectInstalledOutput `
